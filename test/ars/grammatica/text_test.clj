...
(ns  ^{:author "Roger Grantham"}
    ars.grammatica.text-test
  (:use [ars.grammatica.text]
        [clojure.test]))

(deftest normalize-latin-test
  (is (= "iuIVV" (normalize-latin "jvJVU"))))

(deftest remove-macrons-test
  (is (= "aeiouAEIOU" (remove-macrons "āēīōūĀĒĪŌŪ"))))


(deftest slurp-resource-test
  (is (not (empty (slurp-resource "sql/create_lat_lex_table.sql")))))